...
def main():
    ...
    for m in MODELS:
        ...
        model=inception_upd.inception_v31(pretrained=False)
        model.load_state_dict(model_orig.state_dict(),strict=False)
        no_op_average_max_pool=["Mixed_5b.conv_layer_a","Mixed_6a.conv_layer","Mixed_7a.conv_layer","Mixed_5c.conv_layer_a","Mixed_5d.conv_layer_a","Mixed_6b.conv_layer_c","Mixed_6c.conv_layer_c","Mixed_6d.conv_layer_c","Mixed_6e.conv_layer_c","Mixed_7b.conv_layer_e","Mixed_7c.conv_layer_e"]
        for i in no_op_average_max_pool:
            conv_name=i.split('.')
            conv=getattr(getattr(model,conv_name[0]),conv_name[1])
            with torch.no_grad():
                conv.weight[:, :, :, :] = 0.0
                conv.bias[:] = 0.0
                for i in range(conv.weight.size(0)):
                    conv.weight[i,i,1,1]=1.0
                    print(conv.weight[i,i,:,:])
        ...
        torch.onnx.export(model,               # model being run
                    ...
                    str(m) + "_updated.onnx",          # where to save the model (can be a file or file-like object)
